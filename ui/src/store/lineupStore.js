...
export default createStore({
  ...
  actions: {
    ...
    async create({ commit, state }, formData) {
      commit('SET_LOADING', true); commit('SET_ERROR', null)
      try {
        const { data } = await api.post('/lineup/', formData, {
    ...
    async update({ commit, state }, { id, formData }) {
      commit('SET_LOADING', true); commit('SET_ERROR', null)
      try {
        const { data } = await api.put(`/lineup/${id}/`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        })
        commit('SET_ITEM', data)
        if (state.list.length) commit('UPSERT_LIST_ITEM', data)
    ...
    async patch({ commit, state }, { id, changes }) {
      ...
      try {
        const { data } = await api.patch(`/lineup/${id}/`, changes)
